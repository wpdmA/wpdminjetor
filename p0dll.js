...
const delay=ms=>new Promise(e=>setTimeout(e,ms)),
...
// 🔧 Corrigida: agora para no primeiro achado
function findAndClickByText(t){
    const elements=document.querySelectorAll("button, div, span");
    for(const e of elements){
        if(e.textContent.trim().toLowerCase()===t.toLowerCase()){
            e.click();
            e.blur();
            return true; // só o primeiro
        }
    }
    return false;
}

// 🔧 Alternativa fixa: sempre clica na resposta correta
function clickCorrectAnswer(){
    const elements=document.querySelectorAll("button, div, span");
    for(const e of elements){
        if(e.textContent.trim()==="Resposta correta."){
            e.click();
            e.blur();
            return true;
        }
...
function findAndClickByClass(t){
    ...
    if(e){
        e.click();
        e.blur();
        if(e.textContent.includes("Mostrar resumo")){
            sendToast("Concluido - WeLL ⛄️",3e3),
            playAudio("https://r2.e-z.host/4d0a0bea-60f8-44d6-9e74-3032a64a9f32/4x5g14gj.wav")
        }
        return true
    } else {
        // agora tenta clicar só na correta
        return clickCorrectAnswer();
    }
}

async function loadScript(t,l){
